...
export function NewGameInstall(props: {
  ...
  const updateNewInstallShaderpack = (
    name: string,
    loader: string,
    checked: boolean
  ) => {
    if (!checked)
      return setNewInstall((prev) =>
        toggleGameInstallShaderpack(prev, { name, url: '' }, !!checked)
      )
    return queryClient
      .ensureQueryData(modrinthProjectVersionQuery(name))
      .then((data) => {
        ...
        const shaderpack: GameShaderpack = {
          name,
          url: version?.files?.[0]?.url ?? '',
        }
        if (shaderpack.url)
          return setNewInstall((prev) =>
            toggleGameInstallShaderpack(prev, shaderpack, !!checked)
          )
      })
  }
      ...
      >
        ...
        {versionManifests.isError ? (
          ...
          <ModalBody bgColor="rgba(0, 0, 0, 0.6)">
            ...
            {getGameInstalShaderpackLoaderName(newInstall) !==
              ShaderpackLoaderName.none && (
              <List marginTop="1rem">
                {defaultShaderpacks.map((name) => (
                  <ListItem key={name}>
                    <Checkbox
                      isChecked={
                        !!newInstall?.shaderpacks?.find((x) => x.name == name)
                      }
                      onChange={(e) =>
                        updateNewInstallShaderpack(
                          name,
                          getGameInstalShaderpackLoaderName(newInstall),
                          e.target.checked
                        )
